...
export class MarpidianSettingTab extends PluginSettingTab {
  ...
  display(): void {
    ...
    if (this.plugin.marpCliAvailable) {
      cliSetting.setDesc(
        'Détectée. Les exports PDF et PNG sont disponibles dans la vue Marpidian.'
      )
    } else {
      cliSetting
        ...
        .addButton((btn) =>
          btn
            ...
            .onClick(() =>
              ...
              ).shell.openExternal('https://github.com/marp-team/marp-cli')
            )
        )
    }
    ...
    new Setting(containerEl)
      ...
      .setDesc(
        'Dossier cible pour les nouveaux imports (les thèmes existants ne sont pas déplacés).'
      )
      .addText((text) =>
        text
    ...
    new Setting(containerEl)
      ...
      .setDesc(
        'Dossier cible pour les exports PDF et PNG (chemin relatif depuis la racine du vault).'
      )
      .addText((text) =>
        text
          ...
          .onChange(async (value) => {
            this.plugin.settings.exportDir =
              value.trim() || '.marpidian-exports'
            await this.plugin.saveSettings(false)
          })
      )
    ...
    this.plugin.settings.themes.forEach((entry) => {
            ...
            new ConfirmDeleteModal(this.plugin.app, name, async () => {
              try {
                ...
                if (
                  ...
                  new Notice(
                    `[Marpidian] Erreur lors de la suppression : ${e?.message ?? e}`
                  )
                  return
                }
              }
